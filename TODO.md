...
- [x] Re-verify upload modal control behavior
...
- [x] Validate script execution path in PowerShell (`.\push.bat`)
...
- [x] Re-test `/api/chat` method guard, payload guard, and success path
...
- [x] Re-test `/api/chat` method/payload/normal flow and capture behavior

## New task: Switch chat provider to Blackbox paid API
- [x] Update `api/chat.js` env resolution to prioritize `BLACKBOX_*` vars
- [x] Keep retry + fallback behavior intact with Blackbox model chain
- [x] Improve config error text to include Blackbox env names
- [x] Re-test `/api/chat` (GET/invalid POST/normal POST) after restart
